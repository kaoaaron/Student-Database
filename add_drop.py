...
with open(input_filename) as f:
	for row in csv.reader(f):
		...
		try:
			if(row[0] == 'ADD'):
				insert_statement = cursor.mogrify("insert into enrollments values(%s, %s, %s, %s);", (row[1], row[2], row[3], row[4]))
			elif(row[0] == 'DROP'):
				insert_statement = cursor.mogrify("delete from enrollments where (student_id = %s and student_name = %s and course_code = %s and term_code = %s);", (row[1], row[2], row[3], row[4]))
			
			cursor.execute(insert_statement)
			
			if(row[0] == 'ADD'):
				insert_statement = cursor.mogrify("insert into students values(%s, %s);", (row[2], row[1]))
				cursor.execute(insert_statement)
